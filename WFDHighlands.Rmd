...
<!--------------------Best model now 14 including covariates for detectability (wildness, wind and air surface temp------------------->
<!--------- Adding detectability covariates seems to change the sign of the 'wet' predictors - this is weird and doesn't represent a biological relationship that we know about - is this a good justification for getting rid? Or am I getting this wrong? Probably... I doubt the fit of these is good but it might be useful. ------------>
<!----------- Lost the plot.................... -------------->
<!------------------- Zeros are likely to be over water because 4-spotted chaser are more territorial over pools. When we include detectability does this make more pool habitats 0? If so, does this make the relationship with pet and 'wet' negative? ------->
...
<!---------- A MacKenzie & Bailey [ref] goodness of fit test doesn't work because there are two many individual occupancy signatures --------------->

```{r modVal}
### Does not work for our model
# #### GOF test hangs at 28000 - no error just keeps going?!
# WFD.gof <- mb.gof.test(WFD.occu.14, nsim = 10)
# WFD.gof
# 
# ###Indeed this hangs also on the server :(

## Below is currently not quite right - occupancy matrix needs summarising over 1 or 2 of the dimensions.... work in progress...
###GOF with parboot:
...
#This works, but the results are not so exciting... the Chisq goodness of fit test indicates significant deviation from the expected distribution. Also Chat is >1, further suggesting lack of fit 
```
<!----------------------- Model results here ------------->
